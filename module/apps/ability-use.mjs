...
/**
 * Expressões regulares para encontrar modificadores de rolagem
 */
const re = {
	faces: /^d\d+$/,
	die: /\d+d\d+[\+|\-]?[\d+]?/,
...
/**
 * Procura um valor a partir da sua tradução.
 * @param {String} value       Chave do objeto ou valor traduzido
 * @param {Object} configKey   Configuração (T20)
 */
...
/**
 * Aplica alterações na rolagem (rolls) de acordo com o efeito
 * @param {Object} ch         Objeto de mudança do Efeito Ativo
 * @param {Number} qty        Quantidade (multiplicador)
 * @param {Object} ef         Efeito Ativo
 * @param {Object} item       Item ou característica que está sendo modificado
 * @param {Object} id         Copia dos dados da rolagem/roll
 * @param {Object} rollMods   Registro dos modificadores de rolagem
 * @param {Object} options    Opções (como máximo/mínimo)
 */
const applyRollChanges = (ch, qty, ef, item, id, rollMods, options) => {
	...
	if (['atributo', 'pericia'].includes(item.type)) {
		...
		rolls = id.rolls.filter(r =>
			((ch.key === "roll" && item.type !== "arma") ||
				...
				["pericia", "atributoAtq", "atributoDano", "tipoDano", "passos", "danoCritico", "critico"].includes(ch.key) ||
				ch.key.match(/\@([^\#]+)\#/))
		);
	}
	...
	for (let r of rolls) {
		...
		// Permite direcionar o efeito para outra rolagem (@some#roll)
		if (ch.key.match(/\@([^\#]+)\#/)) {
			let m = ch.key.match(/@(.*)#(.*)/);
			if (m[1] && m[2]) {
				sourceName = m[1];
...
/**
 * Modifica os dados do item de acordo com o efeito aplicado.
 * @param {Object} ch             Objeto de mudança do Efeito Ativo
 * @param {Number} qty            Quantidade (multiplicador)
 * @param {Object} ef             Efeito Ativo
 * @param {Object} item           Item a ser modificado
 * @param {Object} id             Dados originais do item
 */
const applyItemChanges = (ch, qty, ef, item, id) => {
	...
	if (ch.mode === 0) _campos;
		...
		if (campos[ch.key] && campos[ch.key][1]) {
			if (ch.key === 'duracao') {
...
/* -------------------------------------------- */
/*  Actor Changes (incluindo cargaArcana e cargaMarcial) */
/* -------------------------------------------- */

/**
 * Novo objeto actorFields incluindo cargaArcana e cargaMarcial.
 */
const actorFields = {
	atributo: ["atributo", null],
	treinado: ["treinado", null],
	treino: ["treino", null],
	cargaArcana: ["attributes.cargaArcana.value", null],
	cargaMarcial: ["attributes.cargaMarcial.value", null],
	pv: ["attributes.pv.value", null],
	pm: ["attributes.pm.value", null]
}

/**
 * Modifica os dados do ator conforme os efeitos aplicados.
 * Essa função foi adaptada para permitir modificações em cargaArcana e cargaMarcial.
 * @param {Object} ch         Objeto de mudança do Efeito Ativo
 * @param {Number} qty        Quantidade (multiplicador)
 * @param {Object} ef         Efeito Ativo
 * @param {Object} item       Dados do ator (actor)
 * @param {Object} id         Dados originais
 * @param {Object} ad         Dados adicionais do ator
 */
const applyActorChanges = (ch, qty, ef, item, id, ad) => {
	const campos = actorFields;
	const _campos = {};

	// CUSTOM CHANGES – nenhuma ação padrão
	if (ch.mode === 0) {
		// Pode ser realizada uma avaliação customizada aqui, se necessário
	}
	// MULTIPLY CHANGES
	else if (ch.mode === 1) {
		if (Number(ch.value)) {
			...
			if (Number(temp)) _campos[campos[ch.key][0]] = Number(temp) * (Number(ch.value) * qty);
				...
				_campos[campos[ch.key][0]] = temp.toString().replace(/\d+/, (match) => Number(match) * (Number(ch.value) * qty));
			}
		}
	}
	// ADD CHANGES (incluindo cargaArcana e cargaMarcial)
	else if (ch.mode === 2) {
		if (Number(ch.value)) {
			let temp = eval(`id.${campos[ch.key][0]}`) ?? false;
...
/**
 * Função para verificar se a chave de efeito corresponde a campos especiais (efeito, condição, treino)
 * @param {String} key
 * @returns {Boolean}
 */
...
/**
 * Aplica as alterações nos efeitos a partir do item ou do status do sistema.
 * @param {Object} ch               Objeto de mudança do Efeito Ativo
 * @param {Number} qty              Quantidade (multiplicador)
 * @param {Object} ef               Efeito Ativo
 * @param {Array} optEffectList     Lista de efeitos opcionais do item
 * @param {Array} effectList        Lista final de efeitos a serem aplicados
 * @param {Array} effectChanges     Alterações adicionais em efeitos
 */
const applyEffectChanges = (ch, qty, ef, optEffectList, effectList, effectChanges) => {
	...
	// Caso deseje modificar um efeito existente, pode-se expandir essa lógica.
}

/**
 * Aplica os modificadores de rolagem no item a partir dos rollMods processados
 * @param {Object} item       Item (ou personagem) a ser modificado
 * @param {Object} rollMods   Modificadores aplicados às rolagens
 * @returns {Array}           Array de rolagens modificadas
 */
...
/**
 * Realiza as modificações na rolagem e nos dados clonados do Item/Ator.
 * @param {Object} rolledItem     Item utilizado
 * @param {Object} configuration  Dados submetidos pelo diálogo de habilidade
 */
/**
...
function applyOnUseEffects(rolledItem, configuration = null) {
	...
	// Obter os efeitos ativos do item
	const effectList = item.effects.filter(
		(ef) =>
			(ef.flags.tormenta20.onuse &&
				ef.flags.tormenta20.durationScene &&
				!ef.disabled) ||
	...
	// Ordena os efeitos onUse para dar prioridade aos que usam override
	onUseEffects.sort((a, b) =>
		a.changes.some((ch) => ch.mode == 5) &&
		!b.changes.some((ch) => ch.mode == 5)
			? -1
			: b.changes.some((ch) => ch.mode == 5) &&
			!a.changes.some((ch) => ch.mode == 5)
				? 1
	...
	// Prepara os dados para o chat e os modificadores de rolagem
	for (let ef of onUseEffects) {
		let ouEff = {};
		ouEff.description =
			item.type !== "arma"
				? ef.name
				: item.id == ef.parent.id
					? `${ef.parent.name} - ${ef.name}`
					: ef.sourceName;
		if (["Unknown", actor.name].includes(ouEff.description))
			ouEff.description = ef.name;
		ouEff.cost =
			Number(applied[ef.id]?.custo) * applied[ef.id]?.aplica ||
			applied[ef.id]?.custo;
		ouEff.qty = Number(applied[ef.id]?.aplica) || 1;

		// Se já houver um efeito onUse da mesma origem, soma custo e quantidade
		const existingEff = options.onUseEffects.find(
			(i) => i.description == ouEff.description
		);
		if (existingEff) {
			existingEff.qty += ouEff.qty - 1 || 0;
			existingEff.cost =
				existingEff.cost == "" ? existingEff.cost : Number(existingEff.cost);
			if (ouEff.cost != "" && Number(ouEff.cost))
				existingEff.cost += Number(ouEff.cost);
		} else {
		...
		const temBonusTeste =
			ef.changes.filter((ch) => ["roll", "ataque"].includes(ch.key)).length > 1;

		// Prepara os modificadores dos efeitos onUse
		for (let ch of ef.changes) {
			...
			// Para alterações que afetam os itens ou os atributos:
			// Se a chave existir em itemFields, usa applyItemChanges.
			// Se existir em actorFields ou se for "cargaArcana" ou "cargaMarcial", usa applyActorChanges.
			if (itemFields[ch.key]) {
				applyItemChanges(ch, ouEff.qty, ef, item, id);
			} else if (actorFields[ch.key] || ["cargaArcana", "cargaMarcial"].includes(ch.key)) {
				applyActorChanges(ch, ouEff.qty, ef, item, id, ad);
			} else if (effectFields(ch.key)) {
				applyEffectChanges(ch, ouEff.qty, ef, optEffectList, effectList, effectChanges);
			} else {
				applyRollChanges(ch, ouEff.qty, ef, item, id, rollMods, options);
			}

			changes.forEach(function (efch) {
				if (
					!ef.flags.tormenta20.aumenta ||
					(ef.flags.tormenta20.aumenta &&
						efch.map((i) => i.key).includes(ch.key))
				) {
					if (
						ch.key == "system.tamanho" &&
	...
	// Prepara os dados derivados do item para atualizar os rótulos
	if (item.type != "pericia" && item.type != "atributo") {
		item.prepareDerivedData();
		// Aplica os modificadores aos dados de rolagem
	...
	// Gera a lista de efeitos que serão exibidos no chat
	...
	effectList.forEach(function (ef, index) {
		...
		if (T20.conditions[efl]) {
			...
			children = tempEffect.flags?.tormenta20?.childEffect?.map(
				(ch) => T20.conditions[ch]
			) || [];
		} else {
			...
			tempEffect.duration = !foundry.utils.isEmpty(duration)
				? duration
				: ef.duration;
			...
			if (tempEffect.changes) {
				tempEffect.changes.sort((c, d) =>
					!Number(c.value) || c.key.match(/efeito.\w+/) ? 1 : -1
				);
				tempEffect.changes = tempEffect.changes.reduce((object, ch) => {
					let key = ch.key.match(/efeito.\w+/)
						? ch.key.toString().split(".")[1]
						: ch.key;
							...
							if (ch.mode == 5) {
								...
								if (ch.key.match(/efeito.\w+/)) {
									if (Number(object[idx].value)) {
										object[idx].value =
											Number(object[idx].value) * Number(ch.value);
									} else if (object[idx].value.match(re.die)) {
										object[idx].value = object[idx].value.replace(
											/\d+/,
											(m) => Number(m * ch.value)
										);
									}
								...
								if (ch.key.match(/efeito.\w+/)) {
									...
									object[idx].value =
										Number(object[idx].value) * Number(value);
								}
							}
						} catch (error) {
							if (ch.mode == 2) {
